		...
		* {
		...
		let _sample_columns = {
			'GC Extraction': {
				...
				'Initial': 'Initial<br>(g/ml)',
				'Final': 'Final<br>(ml)',
				'Surrogate': 'Surrogate<br>(ul)',
				'Spike 1': '<span name="spike1">Spike 1</span><br>(ul)',
				'Spike 2': '<span name="spike2">Spike 2</span><br>(ul)',
				'pH': '<span name="ph">pH</span>',
				'%SED': '%<br>SED',
				'NH': 'NH',
				'pH for Hydrolysis': '<span name="ph_hydrolysis">pH 1<sup>*</sup><br>(>12)</span>',
				'pH for Acid': '<span name="ph_acid">pH 2<sup>*</sup><br>(<2)</span>',
				'Color': 'Color',
				'Moist': '%<br>Moist',
				'Comments': 'Comments',
				/*,'QC 1','QC 2'*/
	...
	<table class="solvent">
		<tbody>
			...
			<tr class='reagent_type'>
				<td colspan='3'>`+(reagent_type.replace('Solvent','Solvent(s) & Reagent(s)').replace('Surrogate','Surrogate(s)').replace('Spike','Spike(s)'))+`</td>
			</tr>
		...
		function DisplayFields(data){
			...
			for(let field in data){
				...
				if (ele){
					...
					text = (typeof _PRINT_NAME != 'undefined' && id in _PRINT_NAME && text in _PRINT_NAME[id])? _PRINT_NAME[id][text] : text
					ele.set('text', text)
		...
		function PrintThis(){
			...
			// Hide some columns
			if (_matrix.contains('Aqueous')){
				$$('.not_for_aqueous').setStyle('display', 'none')
				$$('#unit_of_initial').set('text', 'ml')
			}
			else{
	...
	<body>
		<div id="paper" style="position:relative; height:100%">
			<div id="printable" style="position:absolute;">
				<div>
					<span id="Type" style="float:left">Extraction</span>
				...
				<table style="width:100%">
					<tbody>
						<tr>
							<td>Batch ID:</td>
							<td id="Batch_ID"></td>
							<td id="MDL_Study" display-type="label" style="width:70%"></td>
							<td>Tray:</td>
							<td id="Tray"></td>
						</tr>
						<tr>
